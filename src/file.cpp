        ...
        if (INKSCAPE.use_gui()) {
            ...
            if ( sp_version_inside_range( root->version.inkscape, 0, 1, 0, 92 ) ) {
                ...
                if (!root->viewBox_set && need_fix_viewbox) {
                    ...
                    Gtk::Dialog scaleDialog( _("Old Inkscape file detected (90 DPI)"));
...
#else
                    scaleDialog.get_vbox()->pack_start(info, false, false, 20);
#endif
...
#else
                    scaleDialog.get_vbox()->pack_start(backupButton, false, false, 20);
#endif
