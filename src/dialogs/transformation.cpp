...
static void sp_transformation_dialog_present(unsigned page);
...
static void
sp_transformation_dialog_present(unsigned page)
{
    if (!dlg) {
...
sp_transformation_dialog_new(void)
{
    if (!dlg) {
        ...
        g_signal_connect( G_OBJECT(INKSCAPE), "change_selection",
                          G_CALLBACK(sp_transformation_dialog_selection_changed),
                          dlg );
        g_signal_connect( G_OBJECT(INKSCAPE), "modify_selection",
                          G_CALLBACK(sp_transformation_dialog_selection_modified),
                          dlg );
...
sp_transformation_move_relative_toggled(GtkToggleButton *tb, GObject *dlg)
{
    ...
    double const x = sp_unit_selector_get_value_in_pixels(us, ax);
    double const y = sp_unit_selector_get_value_in_pixels(us, ay);
...
sp_transformation_page_move_new(GObject *obj)
{
    ...
    /* Vertical */
    img = sp_icon_new( GTK_ICON_SIZE_LARGE_TOOLBAR,
