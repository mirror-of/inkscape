...
//      float  px=pts[aretes[bord].st].x,py=pts[aretes[bord].st].y;
  ...
  Path::ArcAngles (prevx, prevy, from->descr_data[nPiece].d.a.x,
		   from->descr_data[nPiece].d.a.y,
...
//		nLarge=!(nLarge);
  ...
  dest->ArcTo (nx, ny, from->descr_data[nPiece].d.a.rx,
	       from->descr_data[nPiece].d.a.ry,
	       from->descr_data[nPiece].d.a.angle, nLarge, nClockwise);
  return bord;
}
...
Shape::ReFormeCubicTo (int bord, int curBord, Path * dest, Path * from)
{
  ...
  Path::CubicTangent (ts, sDx, sDy, prevx, prevy,
		      from->descr_data[nPiece].d.c.stDx,
		      from->descr_data[nPiece].d.c.stDy,
		      from->descr_data[nPiece].d.c.x,
		      from->descr_data[nPiece].d.c.y,
		      from->descr_data[nPiece].d.c.enDx,
		      from->descr_data[nPiece].d.c.enDy);
  Path::CubicTangent (te, eDx, eDy, prevx, prevy,
		      from->descr_data[nPiece].d.c.stDx,
		      from->descr_data[nPiece].d.c.stDy,
		      from->descr_data[nPiece].d.c.x,
		      from->descr_data[nPiece].d.c.y,
...
Shape::ReFormeBezierTo (int bord, int curBord, Path * dest, Path * from)
{
  ...
  if (pe == ps)
    {
      ReFormeBezierChunk (px, py, nx, ny, dest, inBezier, nbInterm, from, ps,
			  ts, te);
    }
  else if (ps < pe)
    {
	  ...
	  if (te > 0.9999)
	    {
	      ...
	      for (int i = ps; i <= pe; i++)
		{
		  dest->IntermBezierTo (from->descr_data[i + 1].d.i.x,
					from->descr_data[i + 1].d.i.y);
		}
	      dest->EndBezierTo ();
	    }
	  else
	    {
	      float tx, ty;
	      tx =
		(from->descr_data[pe + 1].d.i.x +
		 from->descr_data[pe].d.i.x) / 2;
	      ty =
		(from->descr_data[pe + 1].d.i.y +
		 from->descr_data[pe].d.i.y) / 2;
	      dest->BezierTo (tx, ty);
	      for (int i = ps; i < pe; i++)
		{
		  dest->IntermBezierTo (from->descr_data[i + 1].d.i.x,
	      ...
	      dest->EndBezierTo ();
	      ReFormeBezierChunk (tx, ty, nx, ny, dest, inBezier, nbInterm,
				  from, pe, 0.0, te);
	    }
	}
      else
	  ...
	  if (te > 0.9999)
	    {
	      float tx, ty;
	      tx =
		(from->descr_data[ps + 1].d.i.x +
		 from->descr_data[ps + 2].d.i.x) / 2;
	      ty =
		(from->descr_data[ps + 1].d.i.y +
		 from->descr_data[ps + 2].d.i.y) / 2;
	      ReFormeBezierChunk (px, py, tx, ty, dest, inBezier, nbInterm,
				  from, ps, ts, 1.0);
	      dest->BezierTo (nx, ny);
      ...
      if (ts > 0.9999)
	{
	  if (te < 0.0001)
	  ...
	  else
	    {
	      ...
	      tx =
		(from->descr_data[pe + 1].d.i.x +
		 from->descr_data[pe + 2].d.i.x) / 2;
	      ty =
		(from->descr_data[pe + 1].d.i.y +
		 from->descr_data[pe + 2].d.i.y) / 2;
	      dest->BezierTo (tx, ty);
	      for (int i = ps; i > pe; i--)
		{
		  dest->IntermBezierTo (from->descr_data[i + 1].d.i.x,
	  ...
	  if (te < 0.0001)
	    {
	      ...
	      tx =
		(from->descr_data[ps + 1].d.i.x +
		 from->descr_data[ps].d.i.x) / 2;
	      ty =
		(from->descr_data[ps + 1].d.i.y +
		 from->descr_data[ps].d.i.y) / 2;
	      ReFormeBezierChunk (px, py, tx, ty, dest, inBezier, nbInterm,
				  from, ps, ts, 0.0);
	      dest->BezierTo (nx, ny);
	      for (int i = ps + 1; i >= pe; i--)
		{
		  dest->IntermBezierTo (from->descr_data[i].d.i.x,
					from->descr_data[i].d.i.y);
		}
	      dest->EndBezierTo ();
	    }
	  else
...
void
Shape::ReFormeBezierChunk (float px, float py, float nx, float ny,
			   ...
			   Path * from, int p, float ts, float te)
{
  ...
  float cx, cy;
  Path::QuadraticPoint ((ts + te) / 2, cx, cy, bstx, bsty, mx, my, benx,
			beny);
  cx = 2 * cx - (px + nx) / 2;
  cy = 2 * cy - (py + ny) / 2;
  dest->BezierTo (nx, ny);
