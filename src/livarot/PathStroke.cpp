		...
		ButtType butt, float miter, bool justAdd)
{
  ...
  if (doClose)
      ...
      while (prevI > 0)
	{
	  ...
	  if (weighted)
	    ...
	    prevW = 1;
	  float dist =
	    (curP.x - prevP.x) * (curP.x - prevP.x) + (curP.y -
						       prevP.y) * (curP.y -
								   prevP.y);
	  if (dist > 0.001)
	    {
    ...
    }
  {
	...
	if (weighted)
	  ...
	  nextW = 1;
	float dist =
	  (curP.x - nextP.x) * (curP.x - nextP.x) + (curP.y -
						     nextP.y) * (curP.y -
								 nextP.y);
	if (dist > 0.001)
	  {
...
void
Path::DoButt (Shape * dest, float width, ButtType butt, vec2 pos, vec2 dir,
...
void
Path::DoJoin (Shape * dest, float width, JoinType join, vec2 pos, vec2 prev,
	      vec2 next, float miter, float prevL, float nextL, int &leftStNo,
	  ...
	  if (typ >= 0)
	    {
	      x = pos.x - l * biss.x;
	      y = pos.y - l * biss.y;
	      RecRound (dest, rightStNo, rightEnNo, x, y, sx, sy, ex, ey, 5.0,
			8);
	    }
	  else
	    {
	      ...
	      int midNo = dest->AddPoint (mx, my);
	      RecRound (dest, rightStNo, midNo, nsx, nsy, sx, sy, mx, my, 5.0,
			8);
	      RecRound (dest, midNo, rightEnNo, nex, ney, mx, my, ex, ey, 5.0,
			8);
	    }
	}
      else
	  ...
	  if (typ >= 0)
	    {
	      x = pos.x + l * biss.x;
	      y = pos.y + l * biss.y;
	      RecRound (dest, leftEnNo, leftStNo, x, y, ex, ey, sx, sy, 5.0,
			8);
	    }
	  else
	    {
	      ...
	      float nex = pos.x + width * biss.x - dec * tbiss.x;
	      float ney = pos.y + width * biss.y - dec * tbiss.y;
	      RecRound (dest, leftEnNo, midNo, nex, ney, ex, ey, mx, my, 5.0,
			8);
	      RecRound (dest, midNo, leftStNo, nsx, nsy, mx, my, sx, sy, 5.0,
			8);
	    }
	}
      else
...
void
Path::RecRound (Shape * dest, int sNo, int eNo, float px, float py, float sx,
		float sy, float ex, float ey, float tresh, int lev)
{
  if (lev <= 0)
    {
      ...
      do
	{
	  ...
	  if (delta <= dTo->nDashAbs - dashAbs)
	      ...
	      if (dashs[dashNo].length > 0)
		{
		  ...
		  dashAbs += delta;
		  pos.x =
		    (dTo->prevP.x * (dTo->nDashAbs - dashAbs) +
		     dTo->curP.x * (dashAbs - oDashAbs)) / (dTo->nDashAbs -
							    oDashAbs);
		  pos.y =
		    (dTo->prevP.y * (dTo->nDashAbs - dashAbs) +
		     dTo->curP.y * (dashAbs - oDashAbs)) / (dTo->nDashAbs -
							    oDashAbs);
		  nw =
