...
Path::AddPoint (NR::Point const &iPt, bool mvto)
{
	...
	if (mvto == false && nbPt > 0 
		&& ((path_lineto *) pts)[nbPt - 1].p == iPt)
		return -1;
	int n = nbPt++;
	sizePt = nextSize;
	((path_lineto *) pts)[n].isMoveTo = 
		mvto ? polyline_moveto : polyline_lineto;
...
Path::PointAt (int piece, double at, NR::Point & pos)
{
	...
	if (typ == descr_moveto) {
		...
		if (bez_st < 0) // pas trouv le dubut de la spline (mauvais)
			return PointAt (piece - 1, 1.0, pos); 
    
		path_descr_bezierto* stB = (path_descr_bezierto*)(descr_data + descr_cmd[bez_st].dStart);
		if (piece > bez_st + stB->nb) // la spline sort du nombre de
									  // commandes autoris
									  // (mauvais)
			return PointAt (piece - 1, 1.0, pos); 
    
		int k = piece - bez_st;
		const NR::Point bStPt=PrevPoint (bez_st - 1);
		if (stB->nb == 1 || k <= 0) {
			...
			if (k == 1) {
				...
				TangentOnBezAt (at, 0.5*(prevI->p +nextI->p), 
								*nextI, *stB, false, pos, tgt, len, rad);
			} else {
				nextI=(path_descr_intermbezierto*)(descr_data+descr_cmd[bez_st + k].dStart);
				prevI=(path_descr_intermbezierto*)(descr_data+descr_cmd[bez_st + k - 1].dStart);
				...
				TangentOnBezAt (at, 0.5* (prevI->p + nextI->p), 
								*nextI,fin, false, pos, tgt, len, rad);
			}
		}
	}
}
