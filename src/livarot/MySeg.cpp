...
void             L_SEG::Distance(L_SEG &is,double &di,int mode)
{
	if ( mode == inters_seg_seg ) {
		...
		if ( L_VAL_Cmp(oe,os) < 0 ) os=oe;
		if ( L_VAL_Cmp(me,ms) < 0 ) ms=me;
		if ( L_VAL_Cmp(ms,os) < 0 ) os=ms;
		di=os;
...
void             L_SEG::Distance(vec2d &iv,double &di,int mode)
{
...
int              L_SEG::Intersect(L_SEG &iu,L_SEG &iv,int mode)
{
	double  iudd,ivdd;
	L_VEC_Cross(iu.d,iu.d,iudd);
	L_VEC_Cross(iv.d,iv.d,ivdd);
	if ( L_VAL_Zero(iudd) <= 0 ) return 0; // cas illicite
	if ( L_VAL_Zero(ivdd) <= 0 ) return 0; // cas illicite
...
L_VEC_Sub(usve,iu.d,ueve);
	...
	if ( L_VAL_Zero(usvsl) <= 0 ) {
		if ( mode == inters_dr_dr || mode == inters_dmd_dr || mode == inters_seg_dr) {
			return inters_colinear+inters_a_st+inters_b_st;
		} else if ( mode == inters_dmd_dmd || mode == inters_seg_dmd || mode == inters_seg_seg ) {
			if ( L_VAL_Zero(dd) > 0 ) {
				return inters_colinear+inters_a_st+inters_b_st;
			} else {
	...
	if ( L_VAL_Zero(det) == 0 ) { // ces couillons de vecteurs sont colineaires
		vec2d iudp;
		iudp.x=iu.d.y;
		iudp.y=-iu.d.x;
		double dist;
		...
		if ( L_VAL_Zero(dist) == 0 ) {
			if ( mode == inters_dr_dr || mode == inters_dmd_dr || mode == inters_seg_dr ) {
				...
				if ( L_VAL_Zero(dd) > 0 ) return inters_colinear;
				double ts;
				L_VEC_Cross(iu.d,usvs,ts);
				if ( L_VAL_Zero(ts) > 0 ) return inters_colinear;
				return 0;
			} else if ( mode == inters_seg_dmd ) {
				if ( L_VAL_Zero(dd) > 0 ) {
					double ts;
				...
				if ( L_VAL_Zero(ts) > 0 && L_VAL_Zero(te) < 0 ) return inters_colinear;
...
L_MAT_MulV(m,usvs,res);
	...
	if ( mode == inters_dr_dr ) {
		...
		if ( i == 0 ) return inters_a_st+inters_b_mi;
		if ( i > 0 ) return inters_a_mi+inters_b_mi;
		return 0;
	} else if ( mode == inters_seg_dr ) {
		int i=L_VAL_Zero(res.x);
		int j=L_VAL_Cmp(res.x,1);
		if ( i == 0 ) return inters_a_st+inters_b_mi;
		if ( j == 0 ) return inters_a_en+inters_b_mi;
		if ( i > 0 && j < 0 ) return inters_a_mi+inters_b_mi;
		return 0;
	} else if ( mode == inters_dmd_dmd ) {
		int i=L_VAL_Zero(res.x);
		int j=-(L_VAL_Zero(res.y));
		// nota : i=0 et j=0 a ete elimine au debut
		if ( i == 0 && j > 0 ) return inters_a_st+inters_b_mi;
		if ( j == 0 && i > 0 ) return inters_a_mi+inters_b_st;
		if ( i > 0 && j > 0 ) return inters_a_mi+inters_b_mi;
		return 0;
	} else if ( mode == inters_seg_dmd ) {
		int i=L_VAL_Zero(res.x);
		...
		// nota : i=0 et j=0 a ete elimine au debut
		if ( i == 0 && k > 0 ) return inters_a_st+inters_b_mi;
		if ( j == 0 && k > 0 ) return inters_a_en+inters_b_mi;
		if ( i > 0 && j < 0 && k == 0 ) return inters_a_mi+inters_b_st;
		if ( i > 0 && j < 0 && k > 0 ) return inters_a_mi+inters_b_mi;
		return 0;
	} else if ( mode == inters_seg_seg ) {
		int i=L_VAL_Zero(res.x);
		int j=L_VAL_Cmp(res.x,1);
		...
		// nota : i=0 et j=0 a ete elimine au debut
		if ( i == 0 && k > 0 && l < 0 ) return inters_a_st+inters_b_mi;
		if ( j == 0 && k > 0 && l < 0 ) return inters_a_en+inters_b_mi;
		if ( k == 0 && i > 0 && j < 0 ) return inters_a_mi+inters_b_st;
		if ( l == 0 && i > 0 && j < 0 ) return inters_a_mi+inters_b_en;
		if ( k > 0 && l < 0 && i > 0 && j < 0 ) return inters_a_mi+inters_b_mi;
		return 0;
	}
...
int              L_SEG::Intersect(L_SEG &iu,L_SEG &iv,vec2d &at,int mode)
{
	double  iudd,ivdd;
	L_VEC_Cross(iu.d,iu.d,iudd);
	L_VEC_Cross(iv.d,iv.d,ivdd);
...
int              L_SEG::IntersectGeneral(L_SEG &iu,L_SEG &iv,vec2d &at,int mode)
{
...
L_MAT_Det(m,det);
	...
	if ( L_VAL_Zero(det) ) { // ces couillons de vecteurs sont colineaires
		return 0; // paralleles
	}
...
int                L_SEG::Contains(vec2d &pos,int mode)
{
	vec2d     sp,ep;
L_VEC_Sub(pos,p,sp);
L_VEC_Sub(sp,d,ep);
	...
	if ( L_VAL_Zero(spl) == 0 ) {
		if ( mode == inters_dr_pt ) return inters_a_mi;
		return inters_a_st;
	}
	if ( L_VAL_Zero(epl) == 0 ) {
		if ( mode == inters_dr_pt || mode == inters_dmd_pt ) return inters_a_mi;
		return inters_a_en;
	}
...
L_VEC_RotCW(perp);
	...
	if ( L_VAL_Zero(ps) == 0 ) { // sur la droite
		if ( mode == inters_dr_pt ) return inters_a_mi;
		L_VEC_Cross(d,sp,ps);
