...
void
Path::ConvertWithBackData (float treshhold)
{
  ...
  if (weighted)
    lastMoveTo = AddPoint (curX, curY, curW, 0, 0.0, true);
  else
  ...
  while (curP < descr_nb)
	...
	{
	  if (weighted)
	    AddForcedPoint (curX, curY, curW, curP, 1.0);
	  else
      ...
      else if (nType == descr_moveto)
	{
	  ...
	  if (nWeight)
	    ...
	    nextW = 1;
	  if (weighted)
	    lastMoveTo = AddPoint (nextX, nextY, nextW, curP, 0.0, true);
	  else
      ...
      else if (nType == descr_cubicto)
	{
	  ...
	  if (weighted)
	    {
	      RecCubicTo (curX, curY, curW, curD->d.c.stDx, curD->d.c.stDy,
			  nextX, nextY, nextW, curD->d.c.enDx, curD->d.c.enDy,
			  treshhold, 8, 0.0, 1.0, curP);
	      AddPoint (nextX, nextY, nextW, curP, 1.0, false);
	    }
	  else
	    {
	      RecCubicTo (curX, curY, curD->d.c.stDx, curD->d.c.stDy, nextX,
			  nextY, curD->d.c.enDx, curD->d.c.enDy, treshhold, 8,
			  0.0, 1.0, curP);
	      AddPoint (nextX, nextY, curP, 1.0, false);
	    }
	  // et on avance
      ...
      else if (nType == descr_arcto)
	{
	  ...
	  if (weighted)
	    {
	      DoArc (curX, curY, curW, nextX, nextY, nextW, curD->d.a.rx,
		     curD->d.a.ry, curD->d.a.angle, curD->d.a.large,
		     curD->d.a.clockwise, treshhold, curP);
	      AddPoint (nextX, nextY, nextW, curP, 1.0, false);
	    }
	  else
	    {
	      DoArc (curX, curY, nextX, nextY, curD->d.a.rx, curD->d.a.ry,
		     curD->d.a.angle, curD->d.a.large, curD->d.a.clockwise,
		     treshhold, curP);
	      AddPoint (nextX, nextY, curP, 1.0, false);
	    }
	  // et on avance
      ...
      else if (nType == descr_bezierto)
	{
	  ...
	  if (nbInterm <= 0)
	      ...
	      for (int k = 0; k < nbInterm - 1; k++)
		{
		  ...
		  if (k > 0)
		    {
		      if (weighted)
			AddPoint (stx, sty, stw, curP - 1 + k, 1.0, false);
		      else
		  ...
		  if (weighted)
		    {
		      RecBezierTo (cx, cy, cw, stx, sty, stw, (cx + dx) / 2,
				   (cy + dy) / 2, (cw + dw) / 2, treshhold, 8,
				   0.0, 1.0, curP + k);
		    }
		  else
		    {
		      RecBezierTo (cx, cy, stx, sty, (cx + dx) / 2,
				   (cy + dy) / 2, treshhold, 8, 0.0, 1.0,
				   curP + k);
		    }
		}
	      {
		...
		if (nbInterm > 1)
		  {
		    if (weighted)
		      AddPoint (stx, sty, stw, curP + nbInterm - 2, 1.0,
				false);
		    else
	  ...
	  if (weighted)
	    AddPoint (nextX, nextY, nextW, curP - 1 + nbInterm, 1.0, false);
	  else
...
void
Path::ConvertForOffset (float treshhold, Path * orig, float off_dec)
{
  if (descr_flags & descr_adding_bezier)
    CancelBezier ();
  if (descr_flags & descr_doing_subpath)
...
Path::Convert (float treshhold)
{
  ...
  if (weighted)
    lastMoveTo = AddPoint (curX, curY, curW, true);
  else
    lastMoveTo = AddPoint (curX, curY, true);
  ...
  while (curP < descr_nb)
	...
	{
	  if (weighted)
	    AddForcedPoint (curX, curY, curW);
	  else
	    AddForcedPoint (curX, curY);
	  curP++;
	}
      else if (nType == descr_moveto)
	{
	  ...
	  if (nWeight)
	    ...
	    nextW = 1;
	  if (weighted)
	    lastMoveTo = AddPoint (nextX, nextY, nextW, true);
	  else
	    lastMoveTo = AddPoint (nextX, nextY, true);
      ...
      else if (nType == descr_cubicto)
	{
	  ...
	  if (weighted)
	    ...
	    {
	      RecCubicTo (curX, curY, curD->d.c.stDx, curD->d.c.stDy, nextX,
			  nextY, curD->d.c.enDx, curD->d.c.enDy, treshhold,
			  8);
	      curD->associated = AddPoint (nextX, nextY, false);
	      if (curD->associated < 0)
		{
      ...
      else if (nType == descr_bezierto)
	{
	  ...
	  if (nbInterm <= 0)
	      ...
	      else
		{
	      ...
	      if (weighted)
		{
		  RecBezierTo (midX, midY, midW, curX, curY, curW, nextX,
			       nextY, nextW, treshhold, 8);
		}
	      else
		{
		  RecBezierTo (midX, midY, curX, curY, nextX, nextY,
			       treshhold, 8);
		}
	    }
	  else if (nbInterm > 1)
	      ...
	      for (int k = 0; k < nbInterm - 1; k++)
		{
		  ...
		  if (k > 0)
		    {
		      if (weighted)
			(intermPoints - 2)->associated =
			  AddPoint (stx, sty, stw, false);
		      else
			(intermPoints - 2)->associated =
			  AddPoint (stx, sty, false);
		...
		if (weighted)
		  (intermPoints - 1)->associated =
		    AddPoint (stx, sty, stw, false);
		else
		  (intermPoints - 1)->associated = AddPoint (stx, sty, false);
	  ...
	  if (weighted)
	    curBD->associated = AddPoint (nextX, nextY, nextW, false);
	  else
	    curBD->associated = AddPoint (nextX, nextY, false);
...
void
Path::ConvertEvenLines (float treshhold)
{
  if (descr_flags & descr_adding_bezier)
    CancelBezier ();
  if (descr_flags & descr_doing_subpath)
  ...
  while (curP < descr_nb)
	...
	{
	  if (weighted)
	    (curD)->associated = AddForcedPoint (curX, curY, curW);
	  else
	    (curD)->associated = AddForcedPoint (curX, curY);
	  curP++;
	}
      else if (nType == descr_moveto)
	{
	  ...
	  if (weighted)
	    {
	      ...
	      {
		float segL =
		  sqrt ((nextX - curX) * (nextX - curX) +
			(nextY - curY) * (nextY - curY));
		if (segL > 4 * treshhold)
		  {
		    for (float i = 4 * treshhold; i < segL;
			 i += 4 * treshhold)
		      {
			AddPoint (((segL - i) * curX + i * nextX) / segL,
				  ((segL - i) * curY + i * nextY) / segL,
				  ((segL - i) * curW + i * nextW) / segL);
		      }
		  }
	      }
	      curD->associated = AddPoint (nextX, nextY, nextW, false);
      ...
      else if (nType == descr_bezierto)
	{
	  ...
	  if (nbInterm <= 0)
		...
		{
		  RecBezierTo (midX, midY, midW, curX, curY, curW, nextX,
			       nextY, nextW, treshhold, 8, 4 * treshhold);
		}
	      else
		{
		  RecBezierTo (midX, midY, curX, curY, nextX, nextY,
		...
		if (weighted)
		  ...
		  {
		    RecBezierTo (cx, cy, stx, sty, (cx + dx) / 2,
				 (cy + dy) / 2, treshhold, 8, 4 * treshhold);
		  }
	      }
	    }
	  if (weighted)
...
void
Path::PrevPoint (int i, float &x, float &y)
{
  if (i < 0)
    return;
...
void
Path::QuadraticPoint (float t, float &ox, float &oy, float sx, float sy,
...
void
Path::CubicTangent (float t, float &ox, float &oy, float sx, float sy,
		    float sdx, float sdy, float ex, float ey, float edx,
		    float edy)
{
  float ax, bx, cx, dx;
  float ay, by, cy, dy;
...
void
Path::ArcAngles (float sx, float sy, float ex, float ey, float rx, float ry,
		 float angle, bool large, bool wise, float &sang, float &eang)
{
...
void
Path::DoArc (float sx, float sy, float ex, float ey, float rx, float ry,
	     float angle, bool large, bool wise, float tresh)
{
  ...
  if (wise)
      ...
      for (float b = sang - 0.1; b > eang; b -= 0.1)
	{
	  ...
	  float ux, uy;
	  ux = drx + ca * rx * cb - sa * ry * sb;
	  uy = dry + sa * rx * cb + ca * ry * sb;
	  AddPoint (ux, uy);
	}
    }
  else
...
void
Path::DoArc (float sx, float sy, float sw, float ex, float ey, float ew,
...
void
Path::RecCubicTo (float sx, float sy, float sdx, float sdy, float ex,
      ...
      if (sC < tresh && eC < tresh)
	{
	  ...
	  if (maxL > 0 && dC > maxL)
		...
		return;
	      ...
	      RecCubicTo (sx, sy, sdx / 2, sdy / 2, mx, my, mdx, mdy, tresh,
			  lev - 1, maxL);
	      AddPoint (mx, my);
	      RecCubicTo (mx, my, mdx, mdy, ex, ey, edx / 2, edy / 2, tresh,
			  lev - 1, maxL);
	    }
	  return;
	}
...
void
Path::RecCubicTo (float sx, float sy, float sw, float sdx, float sdy,
		  float ex, float ey, float ew, float edx, float edy,
		  float tresh, int lev, float maxL)
{
  float dC = sqrt ((ex - sx) * (ex - sx) + (ey - sy) * (ey - sy));
  if (dC < 0.01)
    {
      ...
      if (sC < tresh && eC < tresh)
	{
	  ...
	  if (maxL > 0 && dC > maxL)
		...
		return;
	      ...
	      RecCubicTo (sx, sy, sw, sdx / 2, sdy / 2, mx, my, mw, mdx, mdy,
			  tresh, lev - 1, maxL);
	      AddPoint (mx, my, mw);
	      RecCubicTo (mx, my, mw, mdx, mdy, ex, ey, ew, edx / 2, edy / 2,
			  tresh, lev - 1, maxL);
	    }
	  return;
	}
  ...
  RecCubicTo (sx, sy, sw, sdx / 2, sdy / 2, mx, my, mw, mdx, mdy, tresh,
	      lev - 1, maxL);
  AddPoint (mx, my, mw);
  RecCubicTo (mx, my, mw, mdx, mdy, ex, ey, ew, edx / 2, edy / 2, tresh,
	      lev - 1, maxL);
}

void
Path::RecBezierTo (float px, float py, float sx, float sy, float ex, float ey,
		   float tresh, int lev, float maxL)
{
  if (lev <= 0)
    return;
...
void
Path::RecBezierTo (float px, float py, float pw, float sx, float sy, float sw,
		   float ex, float ey, float ew, float tresh, int lev,
		   float maxL)
{
  if (lev <= 0)
    return;
		  ...
		  float st, float et, int piece)
{
  ...
  RecCubicTo (sx, sy, sdx / 2, sdy / 2, mx, my, mdx, mdy, tresh, lev - 1, st,
	      mt, piece);
  AddPoint (mx, my, piece, mt);
  RecCubicTo (mx, my, mdx, mdy, ex, ey, edx / 2, edy / 2, tresh, lev - 1, mt,
	      et, piece);
}
		   ...
		   float st, float et, int piece)
{
  ...
  mdw = (sw + pw) / 2;
  mt = (st + et) / 2;
  RecBezierTo (mdx, mdy, mdw, sx, sy, sw, mx, my, mw, tresh, lev - 1, st, mt,
	       piece);
  AddPoint (mx, my, mw, piece, mt);
  mdx = (ex + px) / 2;
  mdy = (ey + py) / 2;
  mdw = (ew + pw) / 2;
  RecBezierTo (mdx, mdy, mdw, mx, my, mw, ex, ey, ew, tresh, lev - 1, mt, et,
	       piece);
}
		  ...
		  float st, float et, int piece, offset_orig & orig)
{
  ...
  {
    vec2 os_pos, os_tgt, oe_pos, oe_tgt, n_tgt;
    n_tgt.x = sdx;
    n_tgt.y = sdy;
    orig.orig->PointAndTangentAt (orig.piece,
				  orig.tSt * (1 - st) + orig.tEn * st, os_pos,
				  os_tgt);
    ...
    n_tgt.y = edy;
    orig.orig->PointAndTangentAt (orig.piece,
				  orig.tSt * (1 - et) + orig.tEn * et, oe_pos,
				  oe_tgt);
...
void
Path::Fill (Shape * dest, int pathID, bool justAdd, bool closeIfNeeded,
	    bool invert)
{
  if (dest == NULL)
    return;
  ...
  if (invert)
      ...
      if (back)
	{
	  if (weighted)
		...
		dest->AddPoint (((path_lineto_wb *) pts)[i].x,
				((path_lineto_wb *) pts)[i].y);
	      int lastM = 0;
	      int curP = 1;
	      int pathEnd = 0;
	      bool closed = false;
	      ...
	      while (curP < nbPt)
		{
		  ...
		  if (sbp->isMoveTo == polyline_moveto)
		    ...
		    {
		      if (fabs (sbp->x - prp->x) < 0.00001
			  && fabs (sbp->y - prp->y) < 0.00001)
			{
			}
		      else
			{
			  lEdge =
			    dest->AddEdge (first + pathEnd, first + curP);
			  dest->ebData[lEdge].pathID = pathID;
			  ...
			  pathEnd = curP;
			  if (fabs (sbp->x - lm->x) < 0.00001
			      && fabs (sbp->y - lm->y) < 0.00001)
			    {
			      closed = true;
			    }
			  else
	  ...
	  else
	    {
	      // !invert && back && !weighted
	      for (int i = 0; i < nbPt; i++)
		dest->AddPoint (((path_lineto_b *) pts)[i].x,
				((path_lineto_b *) pts)[i].y);
	      int lastM = 0;
	      int curP = 1;
	      int pathEnd = 0;
	      bool closed = false;
	  ...
	  if (weighted)
	    {
	      // !invert && !back && weighted
	      for (int i = 0; i < nbPt; i++)
		dest->AddPoint (((path_lineto_w *) pts)[i].x,
				((path_lineto_w *) pts)[i].y);
	      int lastM = 0;
	      int curP = 1;
	      int pathEnd = 0;
	      bool closed = false;
	  ...
	  else
	    {
	      // !invert && !back && !weighted
	      for (int i = 0; i < nbPt; i++)
		dest->AddPoint (((path_lineto *) pts)[i].x,
				((path_lineto *) pts)[i].y);
	      int lastM = 0;
	      int curP = 1;
	      int pathEnd = 0;
