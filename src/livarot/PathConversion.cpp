...
void Path::ConvertWithBackData(double treshhold)
{
    ...
    // le moveto
    {
        ...
        if ( firstTyp == descr_moveto ) {
            curX = dynamic_cast<path_descr_moveto *>(descr_cmd[0])->p;
        } else {
            curP = 0;
    ...
    while ( curP < int(descr_cmd.size()) ) {
        ...
        if ( nType == descr_forced ) {
            ...
            path_descr_lineto *nData = dynamic_cast<path_descr_lineto *>(descr_cmd[curP]);
            nextX = nData->p;
            AddPoint(nextX,curP,1.0,false);
            // et on avance
            curP++;
            ...
            path_descr_cubicto *nData = dynamic_cast<path_descr_cubicto *>(descr_cmd[curP]);
            nextX = nData->p;
            ...
            path_descr_arcto *nData = dynamic_cast<path_descr_arcto *>(descr_cmd[curP]);
            nextX = nData->p;
...
void Path::ConvertForOffset(double treshhold,Path* orig,double off_dec)
{
    ...
    while ( curP < int(descr_cmd.size()) ) {
        ...
        if ( nType == descr_forced ) {
            ...
            path_descr_cubicto *nData = dynamic_cast<path_descr_cubicto *>(descr_cmd[curP]);
            nextX = nData->p;
            off_data.piece = descr_cmd[curP]->associated;
            off_data.tSt = descr_cmd[curP]->tSt;
            off_data.tEn = descr_cmd[curP]->tEn;
            ...
            off_data.tEn = descr_cmd[curP]->tEn;
            if ( descr_cmd[curP]->associated >= 0 ) {
                DoArc(curX, nextX, nData->rx, nData->ry, nData->angle, nData->large,
                      nData->clockwise, treshhold, curP, off_data);
            } else {
                DoArc(curX, nextX, nData->rx, nData->ry, nData->angle, nData->large,
                      nData->clockwise,treshhold,curP);
            }
            AddPoint(nextX, curP, 1.0, false);
            // et on avance
            curP++;
            ...
            if ( nbInterm >= 1 ) {
                ...
                for (int k = 0; k < nbInterm - 1; k++) {
                    ...
                    off_data.piece = descr_cmd[ip]->associated;
                    off_data.tSt = descr_cmd[ip]->tSt;
                    off_data.tEn = descr_cmd[ip]->tEn;
                    if ( descr_cmd[ip]->associated >= 0 ) {
                ...
                
                {
                    ...
                    off_data.piece = descr_cmd[curP + 1]->associated;
                    off_data.tSt = descr_cmd[curP + 1]->tSt;
                    off_data.tEn = descr_cmd[curP + 1]->tEn;
                    if ( descr_cmd[curP + 1]->associated >= 0 ) {
                        NR::Point const mx = (cx + dx) / 2;
                       ...
                       int piece, offset_orig& orig)
{
    ...
    bool enInv = false;
    {
        ...
        double n_len;
        double n_rad;
        path_descr_intermbezierto mid(iP);
        path_descr_bezierto fin(iE, 1);
