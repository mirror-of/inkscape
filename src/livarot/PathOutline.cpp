	  ...
	  if (curD > lastM)
	      ...
	      while (curD > lastM)
		{
		  ...
		  if (typ == descr_moveto)
		    ...
		    {
		      PrevPoint (curD - 1, nextX, nextY);
		      rev->CubicTo (nextX, nextY, -sav_descr[curD].d.c.enDx,
				    -sav_descr[curD].d.c.enDy,
				    -sav_descr[curD].d.c.stDx,
				    -sav_descr[curD].d.c.stDy);
		      curX = nextX;
		      curY = nextY;
		      curD--;
		    }
		    ...
		    {
		      PrevPoint (curD - 1, nextX, nextY);
		      rev->ArcTo (nextX, nextY, sav_descr[curD].d.a.rx,
		      ...
		      else
			{
			  ...
			  for (int i = curD; i > nD; i--)
			    rev->IntermBezierTo (sav_descr[i].d.i.x,
						 sav_descr[i].d.i.y);
			  rev->EndBezierTo ();
			  curX = nextX;
			  curY = nextY;
	      ...
	      else
		{
		  ...
		  if (butt == butt_round)
		    {
		      dest->ArcTo (endPos.x + width * endNor.x,
				   endPos.y + width * endNor.y,
				   1.0001 * width, 1.0001 * width, 0.0, true,
				   true);
		    }
		  else if (butt == butt_square)
		    {
		      dest->LineTo (endPos.x - width * endNor.x +
				    width * endButt.x,
				    endPos.y - width * endNor.y +
				    width * endButt.y);
		      dest->LineTo (endPos.x + width * endNor.x +
		    ...
		    {
		      dest->LineTo (endPos.x + width * endButt.x,
				    endPos.y + width * endButt.y);
		      dest->LineTo (endPos.x + width * endNor.x,
				    endPos.y + width * endNor.y);
		    }
		  else
  ...
  while (curP < descr_nb)
      ...
      else if (nType == descr_moveto)
	{
	  ...
	  if (doFirst)
	  ...
	  else
	    {
	      if (closeIfNeeded)
		{
		  if (fabsf (curX - firstP.x) < 0.0001
		      && fabsf (curY - firstP.y) < 0.0001)
		    {
		  ...
		  else
		    {
		      ...
		      TangentOnSegAt (0.0, curX, curY, temp, stPos, stTgt,
				      stTle);
		      TangentOnSegAt (1.0, curX, curY, temp, enPos, enTgt,
				      enTle);
		      RotCWTo (stTgt, stNor);
		      RotCWTo (enTgt, enNor);
			...
			OutlineJoin (dest, pos, curT, stNor, width, join,
		      ...
		      dest->LineTo (enPos.x + width * enNor.x,
				    enPos.y + width * enNor.y);
      ...
      else if (nType == descr_lineto)
	{
	  ...
	  if (doFirst)
		...
		dest->MoveTo (curX + width * stNor.x, curY + width * stNor.y);
	    }
	  else
	    {
	      // jointure
      ...
      else if (nType == descr_cubicto)
	{
	  ...
	  // et on avance
	  TangentOnCubAt (0.0, curX, curY, curD->d.c, false, stPos, stTgt,
			  stTle, stRad);
	  TangentOnCubAt (1.0, curX, curY, curD->d.c, true, enPos, enTgt,
			  enTle, enRad);
	  RotCWTo (stTgt, stNor);
	  RotCWTo (enTgt, enNor);
      ...
      else if (nType == descr_arcto)
	{
	  ...
	  // et on avance
	  TangentOnArcAt (0.0, curX, curY, curD->d.a, stPos, stTgt, stTle,
			  stRad);
	  TangentOnArcAt (1.0, curX, curY, curD->d.a, enPos, enTgt, enTle,
			  enRad);
	  RotCWTo (stTgt, stNor);
	  RotCWTo (enTgt, enNor);
	  ...
	  if (nbInterm <= 0)
	      ...
	      if (doFirst)
		{
		  ...
		  if (skipMoveto)
		    ...
		    dest->MoveTo (curX + width * stNor.x,
				  curY + width * stNor.y);
		}
	      else
		{
	      ...
	      int n_d =
		dest->LineTo (nextX + width * enNor.x,
			      nextY + width * enNor.y);
	      if (n_d >= 0)
		{
	      ...
	      else
		{
	      ...
	      // et on avance
	      TangentOnBezAt (0.0, curX, curY, intermPoints->d.i,
			      bezStart->d.b, false, stPos, stTgt, stTle,
			      stRad);
	      TangentOnBezAt (1.0, curX, curY, intermPoints->d.i,
...
//                                      float  stw=(bw+cw)/2;
		  ...
		  tempi.x = cx;
		  tempi.y = cy;
		  TangentOnBezAt (1.0, stx, sty, tempi, tempb, true, enPos,
				  enTgt, enTle, enRad);
		  RotCWTo (enTgt, enNor);
...
bool
Path::IsNulCurve (path_descr * curD, float curX, float curY)
{
  int typ = curD->flags & descr_type_mask;
  if (typ == descr_lineto)
    {
      if (fabsf (curD->d.l.x - curX) < 0.00001
	  && fabsf (curD->d.l.y - curY) < 0.00001)
	{
	  return true;
	}
      return false;
    ...
    {
      if (fabsf (curD->d.a.x - curX) < 0.00001
	  && fabsf (curD->d.a.y - curY) < 0.00001)
	{
  ...
  else if (typ == descr_bezierto)
    {
      if (curD->d.b.nb <= 0)
	{
	  if (fabsf (curD->d.b.x - curX) < 0.00001
	      && fabsf (curD->d.b.y - curY) < 0.00001)
	    {
	      return true;
	    }
	  return false;
      ...
      else if (curD->d.b.nb == 1)
	{
	  if (fabsf (curD->d.b.x - curX) < 0.00001
	      && fabsf (curD->d.b.y - curY) < 0.00001)
	    {
	      ...
	      if (fabsf (interm->d.i.x - curX) < 0.00001
		  && fabsf (interm->d.i.y - curY) < 0.00001)
		{
		  return true;
		}
	    }
...
void
Path::TangentOnSegAt (float at, float sx, float sy, path_descr_lineto & fin,
		      vec2 & pos, vec2 & tgt, float &len)
{
  float ex, ey;
  ex = fin.x;
  ey = fin.y;
...
void
Path::TangentOnArcAt (float at, float sx, float sy, path_descr_arcto & fin,
		      vec2 & pos, vec2 & tgt, float &len, float &rad)
{
  ...
  if (wise)
	...
	sang += 2 * M_PI;
      ...
      rad =
	len * (tgt.x * tgt.x + tgt.y * tgt.y) / (tgt.x * dtgt.y -
						 tgt.y * dtgt.x);
      tgt.x /= len;
      tgt.y /= len;
    }
...
void
Path::TangentOnCubAt (float at, float sx, float sy, path_descr_cubicto & fin,
		      bool before, vec2 & pos, vec2 & tgt, float &len,
		      float &rad)
{
  float ex, ey;
  ex = fin.x;
...
/*	ax=sdx+edx+2*sx-2*ex;
  ...
  if (l <= 0.0001)
      ...
      if (l <= 0.0001)
	{
      ...
      rad =
	-l * (dder.x * dder.x + dder.y * dder.y) / (dder.x * ddder.y -
						    dder.y * ddder.x);
      tgt.x = dder.x / l;
      tgt.y = dder.y / l;
      if (before)
  ...
  rad =
    -l * (der.x * der.x + der.y * der.y) / (der.x * dder.y - der.y * dder.x);
...
void
Path::TangentOnBezAt (float at, float sx, float sy,
		      path_descr_intermbezierto & mid,
		      path_descr_bezierto & fin, bool before, vec2 & pos,
		      vec2 & tgt, float &len, float &rad)
{
  float ex, ey;
  ex = fin.x;
  ey = fin.y;
...
void
Path::OutlineJoin (Path * dest, vec2 pos, vec2 stNor, vec2 enNor, float width,
...
/*			float   angle=acos(angCo);
	  ...
	  if (width > 0)
	    {
	      dest->ArcTo (pos.x + width * enNor.x, pos.y + width * enNor.y,
	  ...
	  if (l > miter)
	    {
	      ...
	      dest->LineTo (pos.x + miter * biss.x - r * biss.y,
			    pos.y + miter * biss.y + r * biss.x);
	      dest->LineTo (pos.x + miter * biss.x + r * biss.y,
			    pos.y + miter * biss.y - r * biss.x);
	      dest->LineTo (pos.x + width * enNor.x, pos.y + width * enNor.y);
	    }
		     ...
		     int lev)
{
  ...
  // un cubic
  {
    ...
    TangentOnCubAt (0.0, data->x1, data->y1, temp, false, stPos, stTgt, stTle,
		    stRad);
    TangentOnCubAt (0.5, data->x1, data->y1, temp, false, miPos, miTgt, miTle,
		    miRad);
    TangentOnCubAt (1.0, data->x1, data->y1, temp, true, enPos, enTgt, enTle,
		    enRad);
    RotCWTo (stTgt, stNor);
    RotCWTo (miTgt, miNor);
    RotCWTo (enTgt, enNor);
  ...
  if (lev <= 0)
    {
      int n_d =
	data->dest->CubicTo (enPos.x + width * enNor.x,
			     enPos.y + width * enNor.y,
			     stGue * stTgt.x, stGue * stTgt.y,
			     enGue * enTgt.x, enGue * enTgt.y);
      if (n_d >= 0)
	{
  ...
  req.y = miPos.y + width * miNor.y;
  {
    ...
    TangentOnCubAt (0.5, stPos.x + width * stNor.x, stPos.y + width * stNor.y,
...
Path::StdBezierTo (Path::outline_callback_data * data, float tol, float width)
{
  ...
  float stRad, enRad, stTle, enTle;
  TangentOnBezAt (0.0, data->x1, data->y1, tempi, tempb, false, stPos, stTgt,
		  stTle, stRad);
  TangentOnBezAt (1.0, data->x1, data->y1, tempi, tempb, true, enPos, enTgt,
		  enTle, enRad);
		   ...
		   int lev)
{
  ...
  // un cubic
  {
    ...
    TangentOnArcAt (data->d.a.stA, data->x1, data->y1, temp, stPos, stTgt,
		    stTle, stRad);
    TangentOnArcAt ((data->d.a.stA + data->d.a.enA) / 2, data->x1, data->y1,
		    temp, miPos, miTgt, miTle, miRad);
    TangentOnArcAt (data->d.a.enA, data->x1, data->y1, temp, enPos, enTgt,
		    enTle, enRad);
    RotCWTo (stTgt, stNor);
    RotCWTo (miTgt, miNor);
    RotCWTo (enTgt, enNor);
  ...
  enGue *= enTle;
  float sang, eang;
  ArcAngles (data->x1, data->y1, data->x2, data->y2, data->d.a.rx,
  ...
  if (lev <= 0)
	...
	data->dest->CubicTo (enPos.x + width * enNor.x,
			     enPos.y + width * enNor.y,
			     stGue * stTgt.x * scal, stGue * stTgt.y * scal,
			     enGue * enTgt.x * scal, enGue * enTgt.y * scal);
      if (n_d >= 0)
	{
	  data->dest->descr_data[n_d].associated = data->piece;
