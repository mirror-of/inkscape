...
objects_query_fontnumbers (GSList *objects, SPStyle *style_res)
{
    ...
    for (GSList const *i = objects; i != NULL; i = i->next) {
        ...
        if (style->letter_spacing.normal) {
            ...
            letterspacing += style->letter_spacing.computed * Geom::Affine(SP_ITEM(obj)->i2dt_affine()).descrim(); /// \todo FIXME: we assume non-% units here
        ...
        if (style->word_spacing.normal) {
            ...
            wordspacing += style->word_spacing.computed * Geom::Affine(SP_ITEM(obj)->i2dt_affine()).descrim(); /// \todo FIXME: we assume non-% units here
