...
void
SPIFloat::cascade( SPIBase* const parent ) {
...
void
SPIFloat::merge( SPIBase* const parent ) {
    if( const SPIFloat* p = dynamic_cast<const SPIFloat*>(parent) ) {
        if( inherits ) {
...
void
SPIScale24::cascade( SPIBase* const parent ) {
...
void
SPIScale24::merge( SPIBase* const parent ) {
    if( const SPIScale24* p = dynamic_cast<const SPIScale24*>(parent) ) {
        if( inherits ) {
            if( (!set || inherit) && p->set && !(p->inherit) ) {
                set     = p->set;
...
void
SPILength::cascade( SPIBase* const parent ) {
...
void
SPILength::merge( SPIBase* const parent ) {
    if( const SPILength* p = dynamic_cast<const SPILength*>(parent) ) {
        if( inherits ) {
            if( (!set || inherit) && p->set && !(p->inherit) ) {
                set      = p->set;
...
void
SPILengthOrNormal::merge( SPIBase* const parent ) {
...
void
SPIEnum::cascade( SPIBase* const parent ) {
...
void
SPIEnum::merge( SPIBase* const parent ) {
    if( const SPIEnum* p = dynamic_cast<const SPIEnum*>(parent) ) {
        if( inherits ) {
            if( p->set && !p->inherit ) {
                if( !set || inherit ) {
        ...
        if (this->inherit) {
            ...
            if( this->value ) {
                if( name.compare( "font-family" ) == 0 ) {
                    ...
                    // entire string rather around the individula font names.
...
void
SPIString::cascade( SPIBase* const parent ) {
...
void
SPIString::merge( SPIBase* const parent ) {
    if( const SPIString* p = dynamic_cast<const SPIString*>(parent) ) {
        if( inherits ) {
            if( (!set || inherit) && p->set && !(p->inherit) ) {
                set     = p->set;
...
void
SPIColor::cascade( SPIBase* const parent ) {
...
void
SPIColor::merge( SPIBase* const parent ) {
    if( const SPIColor* p = dynamic_cast<const SPIColor*>(parent) ) {
        if( inherits ) {
            if( (!set || inherit) && p->set && !(p->inherit) ) {
                set           = p->set;
...
// Paint is used for 'fill' and 'stroke'. It is also, incorrectly, used for 'color' and
// 'text-decoration-color' (which can only have color values). The latter should use
// SPIColor. SPIPaint perhaps should be derived from SPIColor.
    ...
    if (streq(str, "inherit")) {
        ...
        if ( strneq(str, "url", 3) ) {
            ...
            if(uri == NULL || uri[0] == '\0') {
                ...
                //         value.href->attach(Inkscape::URI(uri));
                //     } catch (Inkscape::BadURIException &e) {
                //         std::cerr << "SPIPaint::read() " << e.what() << std::endl;
                //         value.href->detach();
                //     }
                // }
            }
...
void
SPIPaint::cascade( SPIBase* const parent ) {
...
void
SPIPaint::merge( SPIBase* const parent ) {
    if( const SPIPaint* p = dynamic_cast<const SPIPaint*>(parent) ) {
...
void
SPIPaintOrder::cascade( SPIBase* const parent ) {
...
void
SPIPaintOrder::merge( SPIBase* const parent ) {
    if( const SPIPaintOrder* p = dynamic_cast<const SPIPaintOrder*>(parent) ) {
...
void
SPIFilter::cascade( SPIBase* const parent ) {
...
void
SPIFilter::merge( SPIBase* const parent ) {
    if( const SPIFilter* p = dynamic_cast<const SPIFilter*>(parent) ) {
...
void
SPIDashArray::cascade( SPIBase* const parent ) {
...
void
SPIDashArray::merge( SPIBase* const parent ) {
    if( const SPIDashArray* p = dynamic_cast<const SPIDashArray*>(parent) ) {
        if( inherits ) {
            if( (!set || inherit) && p->set && !(p->inherit) ) {
                set     = p->set;
...
void
SPIFontSize::cascade( SPIBase* const parent ) {
...
void
SPIFontSize::merge( SPIBase* const parent ) {
    if( const SPIFontSize* p = dynamic_cast<const SPIFontSize*>(parent) ) {
        if( p->set && !(p->inherit) ) {
...
void
SPIBaselineShift::cascade( SPIBase* const parent ) {
...
// FIX ME
void
SPIBaselineShift::merge( SPIBase* const parent ) {
    if( const SPIBaselineShift* p = dynamic_cast<const SPIBaselineShift*>(parent) ) {
        if( (!set || inherit) && p->set && !(p->inherit) ) {
            set     = p->set;
            inherit = p->inherit;
...
void
SPITextDecorationLine::cascade( SPIBase* const parent ) {
...
void
SPITextDecorationLine::merge( SPIBase* const parent ) {
    if( const SPITextDecorationLine* p = dynamic_cast<const SPITextDecorationLine*>(parent) ) {
...
void
SPITextDecorationStyle::cascade( SPIBase* const parent ) {
...
void
SPITextDecorationStyle::merge( SPIBase* const parent ) {
    if( const SPITextDecorationStyle* p = dynamic_cast<const SPITextDecorationStyle*>(parent) ) {
        if( inherits ) { // Always inherits... but special rules?
            if( (!set || inherit) && p->set && !(p->inherit) ) {
                set      = p->set;
...
// void
// SPITextDecoration::cascade( SPIBase* const parent ) {
// }

// void
// SPITextDecoration::merge( SPIBase* const parent ) {
// }
