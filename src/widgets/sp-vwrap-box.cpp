...
/* --- prototypes --- */
static void    sp_vwrap_box_class_init    (SPVWrapBoxClass   *klass);
static void    sp_vwrap_box_init          (SPVWrapBox        *vwbox);
static void    sp_vwrap_box_size_request  (GtkWidget          *widget,
					    GtkRequisition     *requisition);
static void    sp_vwrap_box_size_allocate (GtkWidget          *widget,
					    GtkAllocation      *allocation);
static GSList* reverse_list_col_children   (SPWrapBox         *wbox,
...
sp_vwrap_box_get_type (void)
{
  ...
  if (!vwrap_box_type)
	...
	"SPVWrapBox",
	sizeof (SPVWrapBox),
	sizeof (SPVWrapBoxClass),
	(GtkClassInitFunc) sp_vwrap_box_class_init,
	(GtkObjectInitFunc) sp_vwrap_box_init,
        /* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
	(GtkClassInitFunc) NULL,
      };
      
      vwrap_box_type = gtk_type_unique (SP_TYPE_WRAP_BOX, &vwrap_box_info);
    }
...
static void
sp_vwrap_box_class_init (SPVWrapBoxClass *class)
{
  ...
  widget_class->size_request = sp_vwrap_box_size_request;
  widget_class->size_allocate = sp_vwrap_box_size_allocate;

  wrap_box_class->rlist_line_children = reverse_list_col_children;
}

static void
sp_vwrap_box_init (SPVWrapBox *vwbox)
{
...
GtkWidget*
sp_vwrap_box_new (gboolean homogeneous)
{
  SPVWrapBox *vwbox;

  vwbox = SP_VWRAP_BOX (gtk_widget_new (SP_TYPE_VWRAP_BOX, NULL));

  SP_WRAP_BOX (vwbox)->homogeneous = homogeneous ? TRUE : FALSE;
		       ...
		       GtkRequisition *child_requisition)
{
  if (wbox->homogeneous)
    {
      SPVWrapBox *vwbox = SP_VWRAP_BOX (wbox);
...
static gfloat
get_layout_size (SPVWrapBox *this,
		 guint        max_height,
		 guint       *height_inc)
{
...
static void
sp_vwrap_box_size_request (GtkWidget      *widget,
			    GtkRequisition *requisition)
{
  SPVWrapBox *this = SP_VWRAP_BOX (widget);
...
static GSList*
reverse_list_col_children (SPWrapBox       *wbox,
			   SPWrapBoxChild **child_p,
			   GtkAllocation    *area,
...
static void
sp_vwrap_box_size_allocate (GtkWidget     *widget,
			     GtkAllocation *allocation)
{
  SPWrapBox *wbox = SP_WRAP_BOX (widget);
  GtkAllocation area;
