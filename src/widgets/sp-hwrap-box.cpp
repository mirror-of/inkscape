 ...
 *
 * SPHWrapBox: Horizontal wrapping box widget
...
/* --- prototypes --- */
static void    sp_hwrap_box_class_init    (SPHWrapBoxClass   *klass);
static void    sp_hwrap_box_init          (SPHWrapBox        *hwbox);
static void    sp_hwrap_box_size_request  (GtkWidget          *widget,
					    GtkRequisition     *requisition);
static void    sp_hwrap_box_size_allocate (GtkWidget          *widget,
					    GtkAllocation      *allocation);
static GSList* reverse_list_row_children   (SPWrapBox         *wbox,
...
sp_hwrap_box_get_type (void)
{
  ...
  if (!hwrap_box_type)
	...
	"SPHWrapBox",
	sizeof (SPHWrapBox),
	sizeof (SPHWrapBoxClass),
	(GtkClassInitFunc) sp_hwrap_box_class_init,
	(GtkObjectInitFunc) sp_hwrap_box_init,
        /* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
	(GtkClassInitFunc) NULL,
      };
      
      hwrap_box_type = gtk_type_unique (SP_TYPE_WRAP_BOX, &hwrap_box_info);
    }
...
static void
sp_hwrap_box_class_init (SPHWrapBoxClass *class)
{
  GtkObjectClass *object_class;
  GtkWidgetClass *widget_class;
  GtkContainerClass *container_class;
  SPWrapBoxClass *wrap_box_class;
  
  object_class = GTK_OBJECT_CLASS (class);
  widget_class = GTK_WIDGET_CLASS (class);
  container_class = GTK_CONTAINER_CLASS (class);
  wrap_box_class = SP_WRAP_BOX_CLASS (class);
  
  parent_class = gtk_type_class (SP_TYPE_WRAP_BOX);
  
  widget_class->size_request = sp_hwrap_box_size_request;
  widget_class->size_allocate = sp_hwrap_box_size_allocate;

  wrap_box_class->rlist_line_children = reverse_list_row_children;
}

static void
sp_hwrap_box_init (SPHWrapBox *hwbox)
{
...
GtkWidget*
sp_hwrap_box_new (gboolean homogeneous)
{
  SPHWrapBox *hwbox;

  hwbox = SP_HWRAP_BOX (gtk_widget_new (SP_TYPE_HWRAP_BOX, NULL));

  SP_WRAP_BOX (hwbox)->homogeneous = homogeneous ? TRUE : FALSE;

  return GTK_WIDGET (hwbox);
}

static inline void
get_child_requisition (SPWrapBox     *wbox,
		       GtkWidget      *child,
		       GtkRequisition *child_requisition)
{
  if (wbox->homogeneous)
    {
      SPHWrapBox *hwbox = SP_HWRAP_BOX (wbox);
...
static gfloat
get_layout_size (SPHWrapBox *this,
		 guint        max_width,
		 guint       *width_inc)
{
  SPWrapBox *wbox = SP_WRAP_BOX (this);
...
static void
sp_hwrap_box_size_request (GtkWidget      *widget,
			    GtkRequisition *requisition)
{
  SPHWrapBox *this = SP_HWRAP_BOX (widget);
...
static GSList*
reverse_list_row_children (SPWrapBox       *wbox,
			   SPWrapBoxChild **child_p,
			   GtkAllocation    *area,
	    ...
	    gboolean       vexpand)
{
  ...
  for (slist = children; slist; slist = slist->next)
    {
      SPWrapBoxChild *child = slist->data;
      GtkRequisition child_requisition;
...
static void
layout_rows (SPWrapBox    *wbox,
	     GtkAllocation *area)
{
  SPWrapBoxChild *next_child;
  guint min_height;
  gboolean vexpand;
  GSList *slist;
  Line *line_list = NULL;
...
static void
sp_hwrap_box_size_allocate (GtkWidget     *widget,
			     GtkAllocation *allocation)
{
  SPWrapBox *wbox = SP_WRAP_BOX (widget);
  GtkAllocation area;
