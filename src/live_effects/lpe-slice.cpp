...
/** \file
 * LPE <slice> implementation: slice a item with respect to a given line.
 */
/*
 * Authors:
 *   Maximilian Albert
...
void
LPESlice::doAfterEffect (SPLPEItem const* lpeitem)
{
    is_load = false;
...
void LPESlice::sp_clone_style(SPObject *orig, SPObject *dest)
{
...
void
LPESlice::cloneD(SPObject *orig, SPObject *dest, bool reset) 
{
    SPDocument * document = SP_ACTIVE_DOCUMENT;
    if (!document) {
        return;
...
void
LPESlice::toMirror(Geom::Affine transform, bool reset)
{
    SPDocument * document = SP_ACTIVE_DOCUMENT;
    if (!document) {
        return;
