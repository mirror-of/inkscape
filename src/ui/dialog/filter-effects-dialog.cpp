...
public:
    ...
    Glib::ustring get_as_attribute() const
    {
        std::cout << "EntryAttr::get_as_attribute: " << get_text() << std::endl;
        return get_text();
    }
    ...
    void set_from_attribute(SPObject* o)
    {
        const gchar* val = attribute_value(o);
        if(val) {
            std::cout << "EntryAttr::set_from_attribute: " << val << std::endl;
            set_text( val );
        } else {
            set_text( "" );
        }
...
public:
    ...
    {
        std::cout << "show_and_update: new: " << t << " old: " << _current_type
                  << "  " << (ob->getId() ? ob->getId() : "NULL")  << std::endl;
        if(t != _current_type) {
        ...
        if(t >= 0) {
            ...
            _groups[t]->show(); // Do not use show_all()
        }
    ...
    ComponentTransferValues* add_componenttransfervalues(const Glib::ustring& label, int channel);
...
{
public:
    ComponentTransferValues(FilterEffectsDialog& d, int channel)
        : AttrWidget(SP_ATTR_INVALID),
          ...
          _funcNode(NULL)
    {
        std::cout << "CTV::constructor: " << _channel << std::endl;
        set_shadow_type(Gtk::SHADOW_IN);
        add(_box);
        _box.add(_type);
    ...
    SPFeFuncNode* find_node(SPFeComponentTransfer* ct)
    {
        ...
        for(;node;node=node->next){
            ...
            if( (funcNode->channel == SPFeFuncNode::R && _channel == 0) ||
                (funcNode->channel == SPFeFuncNode::G && _channel == 1) ||
                (funcNode->channel == SPFeFuncNode::B && _channel == 2) ||
                (funcNode->channel == SPFeFuncNode::A && _channel == 3) ) {
                found = true;
                break;
            }
        }
    ...
    virtual void set_from_attribute(SPObject* o)
    {
        std::cout << "CTV::set_from_attribute: " << _channel << std::endl;
        // See componenttransfer.cpp
        if(SP_IS_FECOMPONENTTRANSFER(o)) {
            SPFeComponentTransfer* ct = SP_FECOMPONENTTRANSFER(o);
            std::cout << "  We have FECOMPONENTTRANSFER" << std::endl;

            _funcNode = find_node(ct);
            if( _funcNode ) {
                std::cout << "  Correct child found: type: " << _funcNode->type << std::endl;
                _type.set_from_attribute( _funcNode );
            } else {
                std::cout << "  Correct child not found" << std::endl;
                std::cout << "  Adding new SPFeFuncNode" << std::endl;
                ...
                if(prim) {
                    ...
                    std::cout << "ERROR ERROR: primitive not found!" << std::endl;
                }
            }
...
private:
    ...
    {
        std::cout << "CTV::on_type_changed()" << std::endl;

        if(_locked) {
            std::cout << "  LOCKED LOCKED LOCKED LOCKED LOCKED LOCKED LOCKED" << std::endl;
            return;
        }
        SPFilterPrimitive* prim = _dialog._primitive_list.get_selected();
        if(prim) {
            ...
            const int type = _type.get_active_row_number();
            std::cout << "  new type: " << type
                      << " (" << _type.get_as_attribute() << ")" << std::endl;

            _funcNode->setAttribute( "type", _type.get_as_attribute().c_str() );

            DocumentUndo::done(prim->document, SP_VERB_DIALOG_FILTER_EFFECTS, _("New transfer function type"));
            update();
            //set_from_attribute( prim );
    ...
    void update()
    {
        std::cout << "CTV::update()" << std::endl;
        SPFilterPrimitive* prim = _dialog._primitive_list.get_selected();
        if(prim && _funcNode) {
            std::cout << " Calling show_and_update: " << std::endl;
...
public:
    virtual Glib::ustring get_as_attribute() const
    {
        std::cout << "CTV::get_as_attribute" << std::endl;
        return "";
    }
...
FilterEffectsDialog::ComponentTransferValues* FilterEffectsDialog::Settings::add_componenttransfervalues(const Glib::ustring& label, int channel)
    {
...
void FilterEffectsDialog::init_settings_widgets()
{
    ...
    _settings->add_componenttransfervalues(_("R:"), 0);
    _settings->add_componenttransfervalues(_("G:"), 1);
    _settings->add_componenttransfervalues(_("B:"), 2);
    _settings->add_componenttransfervalues(_("A:"), 3);
