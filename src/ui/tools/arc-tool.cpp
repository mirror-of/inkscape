...
void ArcTool::drag(Geom::Point pt, guint state) {
    ...
    if (state & GDK_CONTROL_MASK) {
        ...
        this->message_context->setF(Inkscape::IMMEDIATE_MESSAGE, _("<b>Ellipse</b>: <tt>%s</tt> &#215; <tt>%s</tt> (constrained to ratio <tt>%d</tt>:<tt>%d</tt>); with <b>Shift</b> to draw around the starting point"), xs.c_str(), ys.c_str(), ratio_x, ratio_y);
    } else {
        this->message_context->setF(Inkscape::IMMEDIATE_MESSAGE, _("<b>Ellipse</b>: <tt>%s</tt> &#215; <tt>%s</tt>; with <b>Ctrl</b> to make square or integer-ratio ellipse; with <b>Shift</b> to draw around the starting point"), xs.c_str(), ys.c_str());
    }
}
