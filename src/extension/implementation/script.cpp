            ...
            for(auto y = selected_items.begin(); y != selected_items.end(); ++y){
                ...
                if((elemref = desktop->getDocument()->getObjectById((*y)))){
                    ...
                    if (itemselected) {
                        ...
                        //first implementation in extension "Gears"
                        //TODO: fix getObjectsBySelector to return matched elements and allow next code
                        std::vector< SPObject * > satellites = desktop->getDocument()->getObjectsBySelector("select");
                        for(auto j = satellites.begin(); j != satellites.end(); ++j){
                            ...
                            if (satelliteselected) {
                                ...
                                Glib::ustring classAttr = satelliteselected->getRepr()->attribute("class");
                                auto k = classAttr.find("select");
                                if (k != Glib::ustring::npos) {
                                    classAttr.erase(k, 6);
                                }
                                if (k != Glib::ustring::npos && classAttr[k] == ' ') {
                                    classAttr.erase(k, 1);
                                }
                                satelliteselected->getRepr()->setAttribute("class", classAttr);
...
void Script::copy_doc (Inkscape::XML::Node * oldroot, Inkscape::XML::Node * newroot)
{
    ...
    oldroot->mergeFrom(newroot, "id", false, true);
    return;
}
