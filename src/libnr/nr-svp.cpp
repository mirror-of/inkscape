...
nr_svl_build_finish_segment (NRSVLBuild *svlb)
{
    if (svlb->refvx) {
        ...
        *svlb->svl = nr_svl_insort(*svlb->svl, new_svl);
    }
    svlb->refvx = NULL;
}
...
nr_svl_build_lineto (NRSVLBuild *svlb, float x, float y)
{
    ...
    if (y != svlb->sy) {
        ...
        svlb->refvx = vertex;
        /* Stretch bbox */
        nr_rect_d_union_xy (&svlb->bbox, x, y);
        svlb->sx = x;
        svlb->sy = y;
    } else if (x != svlb->sx) {
        NRFlat *flat;
        ...
        *svlb->flats = nr_flat_insort (*svlb->flats, flat);
        svlb->sx = x;
        /* sy = y ;-) */
    }
}
...
NRSVL *
nr_svl_insort (NRSVL *start, NRSVL *svl)
{
    NRSVL * s, * l;
...
nr_svl_move_sorted (NRSVL *start, NRSVL *svl)
{
    ...
    if (s) {
        s->next = nr_svl_insort (svl->next, svl);
        return start;
    }

    return nr_svl_insort (start, svl);
}
...
NRFlat *
nr_flat_insort (NRFlat *start, NRFlat *flat)
{
    NRFlat *s, *l;
