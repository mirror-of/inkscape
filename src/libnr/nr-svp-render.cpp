...
static NRSliceL *nr_slice_insort_l (NRSliceL *start, NRSliceL *slice);
               ...
               void (* run) (unsigned char *px, int len, int c0_24, int s0_24, void *data), void *data)
{
    ...
    while ((nsvl) && (nsvl->bbox.y0 <= y0)) {
        ...
        if (nsvl->bbox.y1 > y0) {
            ...
            slices = nr_slice_insort_l (slices, newslice);
        }
        nsvl = nsvl->next;
    }
...
static NRSliceL *
nr_slice_insort_l (NRSliceL * start, NRSliceL * slice)
{
    NRSliceL * s, * l;
