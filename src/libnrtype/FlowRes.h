...
public:
	...
	typedef struct flow_styled_letter {
		...
		bool           invisible;   // invisible letter (=newline for example)
	} flow_styled_letter;
	...
	typedef struct flow_styled_chunk {
		...
		bool					 rtl;       // line rtl (may be != from its spans rtl)
		double         x_st,x_en,y,spacing; // position/ letterspacing
		double         ascent,descent,leading; // dimensions
		text_holder*   mommy;     // the paragraph that gave birth to this line (this gives access to the one_flow_src
		                          // bounding this line)
	} flow_styled_chunk;
	...
	void               Print(SPPrintContext *ctx,
				 ...
				 NRMatrix const &ctm);
	//  - transform in a big fat sp-curve
