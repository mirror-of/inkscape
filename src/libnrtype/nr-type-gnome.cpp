...
static unsigned int nr_typeface_gnome_attribute_get (NRTypeFace *tf, 
						     const gchar *key, 
						     gchar *str, 
						     unsigned int size);
...
static NR::Point nr_typeface_gnome_glyph_advance_get (NRTypeFace *tf, unsigned int glyph, unsigned int metrics);
...
nr_typeface_gnome_setup (NRTypeFace *tface, NRTypeFaceDef *def)
{
    ...
    tfg->face = gnome_font_face_find ((guchar *)def->name);
    tfg->fonts = NULL;
...
nr_type_gnome_typefaces_get (NRNameList *typefaces)
{
	...
	typefaces->names = nr_new (guchar *, typefaces->length);
	pos = 0;
	for (l = fl; l; l = l->next) {
	    typefaces->names[pos++] = (guchar *) l->data;
	}
    } else {
	typefaces->length = 0;
...
nr_type_gnome_families_get (NRNameList *families)
{
	...
	families->names = nr_new (guchar *, families->length);
	pos = 0;
	for (l = fl; l; l = l->next) {
...
void
nr_type_gnome_build_def (NRTypeFaceDef *def, const guchar *name, const guchar *family)
{
    def->type = NR_TYPE_TYPEFACE_GNOME;
    def->name = g_strdup ((gchar *)name);
    def->family = g_strdup ((gchar *)family);
    def->typeface = NULL;
}
...
static unsigned int
nr_typeface_gnome_attribute_get (NRTypeFace *tf, const gchar *key, gchar *str, unsigned int size)
{
    NRTypeFaceGnome *tfg;
    const gchar *val;
    int len;
	...
	if (gnome_font_face_is_italic (tfg->face)) {
	    ...
	    if (strstr ((gchar *)name, "oblique") || strstr ((gchar *)name, "Oblique") || (strstr ((gchar *)name, "OBLIQUE"))) {
		val = "oblique";
	    } else {
		val = "italic";
	    }
...
static NR::Point
nr_typeface_gnome_glyph_advance_get (NRTypeFace *tf, unsigned int glyph, unsigned int metrics)
{
    NRTypeFaceGnome *tfg;
    NR::Point adv;
