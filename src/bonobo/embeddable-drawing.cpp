...
sp_embeddable_drawing_get_type (void)
{
    ...
    if (!type) {
        ...
        type = bonobo_x_type_unique ( BONOBO_CANVAS_COMPONENT_TYPE, 
                                      NULL, NULL, 0, &info );
    }
    return type;
}
                                ...
                                gpointer data )
{
    ...
    drawing->drawing = 
        (GnomeCanvasGroup *) gnome_canvas_item_new ( gnome_canvas_root (canvas),
...
#if 0
    drawing->drawing = 
        (GnomeCanvasGroup *) gnome_canvas_item_new ( drawing->drawing,
                                                     SP_TYPE_CANVAS_TRANSLATOR,
                                                     NULL );
    art_affine_scale (affine, 0.1, 0.1);
    affine[4] = 100.0;
    affine[5] = 100.0;
    gnome_canvas_item_affine_absolute ( GNOME_CANVAS_ITEM (drawing->drawing), 
                                        affine );
#endif
...
void
sp_embeddable_drawing_new_doc ( BonoboCanvasComponent * component, 
                                gpointer data )
{
    SPEmbeddableDrawing * drawing;
