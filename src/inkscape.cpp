...
void Application::add_inkscape_css()
{
    ...
    if (customthemename.find("inkscapecustom_sys") == std::string::npos) { // if is systemwide defaults relay on
                                                                           // style.css
        if (customthemename.find("inkscapecustom_") != std::string::npos) {
            customthemename = customthemename.substr(15);
...
void Application::add_gtk_css()
{
    ...
    if (settings) {
        ...
        if (gtkthemename != "") {
            if (gtkthemename.find("inkscapecustom_") != std::string::npos) {
                // we use adwaita as base of all custom CSS, seems more standar than user default theme than can make
                // unwanted results
                if (gtkthemename.find(":dark") != std::string::npos ||
                    gtkthemename.find("_dark") != std::string::npos ||
                    prefs->getBool("/theme/preferDarkTheme", gtkApplicationPreferDarkTheme)) {
            ...
            prefs->setString("/theme/gtkTheme", prefs->getString("/theme/defaultTheme"));
        }
