...
#define DIRECT_DRAW
gint const UPDATE_PRIORITY = GDK_PRIORITY_REDRAW + 10;  // must be same or lower priority than GDK_PRIORITY_REDRAW or else canvas, rulers, & status bar will update at very low rate or not at all (they'll starve)
...
//#define METHOD2
#ifndef METHOD2
    // cleaner, but not thread safe because of using cairo_surface_set_device_scale()
