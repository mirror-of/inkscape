...
static void sp_offset_get_tangent (SPOffset const *offset, gdouble t, NRPoint *p);
...
sp_offset_get_type (void)
{
	...
	if (!offset_type) {
		...
		offset_type = g_type_register_static (SP_TYPE_SHAPE, "SPOffset", &offset_info, (GTypeFlags)0);
	}
	return offset_type;
}
...
gchar*  liv_svg_dump_path2 (Path* path)
{
	GString *result;
...
bool  vectors_are_clockwise(double ax,double ay,double bx,double by,double cx,double cy)
{
  ...
  if ( ab_c <= -1.0 ) ab_a=M_PI;
  if ( ab_c >= 1.0 ) ab_a=0;
  if ( ab_s < 0 ) ab_a=2*M_PI-ab_a;
  double   bc_a=acos(bc_c);
  if ( bc_c <= -1.0 ) bc_a=M_PI;
  if ( bc_c >= 1.0 ) bc_a=0;
  if ( bc_s < 0 ) bc_a=2*M_PI-bc_a;
  double   ca_a=acos(ca_c);
  if ( ca_c <= -1.0 ) ca_a=M_PI;
  if ( ca_c >= 1.0 ) ca_a=0;
  if ( ca_s < 0 ) ca_a=2*M_PI-ca_a;

  double lim=2*M_PI-ca_a;

  if ( ab_a < lim ) return true;    
  return false;
}

double      sp_offset_distance_to_original(SPOffset* offset,double px,double py)
{
  if ( offset == NULL || offset->originalPath == NULL || ((Path*)offset->originalPath)->descr_nb <= 1 ) return 1.0;
  double   dist=1.0;
  Shape* theShape=new Shape;
  Shape* theRes=new Shape;
  ...
  if ( theRes->nbAr <= 1 ) {
    ...
    for (int i=0;i<theRes->nbPt;i++) {
      if ( theRes->pts[i].dI+theRes->pts[i].dO > 0 ) {
        ...
        double ndist=sqrt((px-nx)*(px-nx)+(py-ny)*(py-ny));
        if ( ptSet == false || fabs(ndist) < fabs(ptDist) ) {
          ...
          do {
            ...
            nex/=nlen;
            ney/=nlen;
            if ( theRes->aretes[pb].en == i ) {prx=-prx;pry=-pry;}
            if ( theRes->aretes[cb].en == i ) {nex=-nex;ney=-ney;}
    ...
    for (int i=0;i<theRes->nbAr;i++) {
     ...
     if ( arSet || ptSet ) {
       if ( arSet == false ) arDist=ptDist;
       if ( ptSet == false ) ptDist=arDist;
       if ( fabs(ptDist) < fabs(arDist) ) dist=ptDist; else dist=arDist;
     }
   }
