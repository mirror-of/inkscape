		 ...
		 NR::Point const &tHat1, NR::Point const &tHat2)
{
	...
	for (unsigned i = 0; i < len; i++) {
		...
		/* Additional offset to the data point from the predicted point if we were to set
		   bezier[1] to bezier[0] and bezier[2] to bezier[3]. */
	...
	/* TODO: Check whether this special-casing is necessary now
	   that NewtonRaphsonRootFind handles non-positive
	   denominator. */
 ...
 * a better assignment of parameter values for the points.
 *
...
NewtonRaphsonRootFind(BezierCurve Q, NR::Point const &P, gdouble u)
{
	...
	if(denominator > 0.) {
		...
		/* Using Newton-Raphson would move in the wrong direction
		   (towards a local maximum rather than local minimum),
		   so we move an arbitrary amount in the right direction. */
		if(numerator > 0.) {
			improved_u = u * .98 - .01;
		} else if(numerator < 0.) {
 ...
 */
static NR::Point
bezier_pt (unsigned degree, NR::Point const V[], gdouble t)
{
	/** Pascal's triangle. */
 ...
 */
static void
chord_length_parameterize(NR::Point const d[], gdouble u[], unsigned len)
{
 ...
 *	fitted curve, and (if this maximum error is non-zero) set
 *      \a *splitPoint to the corresponding index.
 *
 *	Requires: 2 <= len.
 *
 */
static gdouble
compute_max_error (NR::Point const d[], double const u[], unsigned const len, BezierCurve const bezCurve, unsigned *splitPoint)
{
	g_assert (2 <= len);
	unsigned const last = len - 1;
	g_assert (bezCurve[0] == d[0]);
