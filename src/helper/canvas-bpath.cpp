...
sp_canvas_bpath_update (SPCanvasItem *item, double *affine, unsigned int flags)
{
    ...
    {
      NR::Matrix   tempMat(affine[0],affine[1],affine[2],affine[3],affine[4],affine[5]);
      thePath->LoadArtBPath(cbp->curve->bpath,tempMat,true);
    }
...
#ifdef bpath_liv
  {
    NR::Point const thePt = p;
        ...
        *actual_item = item;
		...
		if (cbp->stroke_shp ) {
			if (cbp->stroke_shp->PtWinding(thePt) > 0 ) {
        *actual_item = item;
        return 0.0;
      }
      dist=cbp->stroke_shp->Distance(thePt);
      return dist;
		}
    if (cbp->fill_shp) {
      dist=cbp->fill_shp->Distance(thePt);
      return dist;
    }
  }
#else
