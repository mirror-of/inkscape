...
def isNondrawing(node):
    if node.tag in [ inkex.addNS('defs', 'svg'), 'defs', inkex.addNS('metadata', 'svg'), 'metadata' ]:
        return True
    if node.xpath('namespace-uri()') not in [ 'http://www.w3.org/2000/svg', '']:
        return True
    return False
