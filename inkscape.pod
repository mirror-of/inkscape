...
Wacom tablets do work with Inkscape if they are configured to act as
...
Note that we do not support pressure/tilt sensitivity or any
tablet-specific features yet.


=head1 BUGS

Many bugs are known; please refer to the website for reviewing the
reported ones and to report newly found issues.  For bugs that cannot be
easily fixed, here are the workarounds:
