...
Inkscape - an SVG (Scalable Vector Graphics) vector editor.
...
shapes, bezier paths, freehand drawing, multiline text, 
alpha blending, transforms, gradients, node
editing, SVG-to-PNG export, grouping, patterns, and more.  The interface is
...
standard, and since its files are text/xml instead of binary, it will be
...
Exported area in SVG pixels (anonymous length units normally used in Inkscape SVG). The default is to export the entire document page. The 0,0 is
the lower-left corner.

=item B<-b> I<COLOR>, B<--export-background>=I<COLOR>

Background color of exported PNG. This may be any SVG supported color string, for example "#ff007f" or "rgb(255, 0, 128)". If not set, the page color set in Inkscape in the Document Options dialog will be used (stored in the pagecolor= attribute of sodipodi:namedview).

=item B<-d> I<DPI>, B<--export-dpi>=I<DPI>

The resolution used for bitmap export. The default is 72 which corresponds to 1 SVG pixel exporting to 1 bitmap pixel. This value overrides the DPI hint if used with --export-use-hints.

=item B<-e> I<FILENAME>, B<--export-png>=I<FILENAME>

Specify the filename for PNG export. If exists, the file will be overwritten without asking.
...
The id attribute value of the object that you want to export from the document. Exported is the bounding box of the object, so with this option, --export-area is ignored. 
...
Use export filename and DPI hints stored in the exported object (only with --export-id). These hints are set automatically when you export selection from within Inkscape. So, for example, if you export a shape with id="path231" as /home/me/shape.png at 300 dpi from document.svg using Inkscape GUI and then save the document, later you will be able to reexport that shape to the same file with the same resolution simply with 

inkscape -i path231 -t document.svg 

If you use --export-dpi, --export-width, or --export-height with this option, the DPI hint will be ignored and the value from the command line will be used. If you use --export-png with this option, the filename hint will be ignored and the filename from the command line will be used. 
...
=item B<-x>, B<--bitmap-icons>

Use bitmap (XPM) icons instead of SVG ones. By default, Inkscape raterizes icons on-the-fly 
from share/inkscape/icons/icons.svg. Use this option to turn this off if there is any problem with SVG icons. 

=item B<-y> I<VALUE>, B<--export-background-opacity>=I<VALUE>

Opacity of the background of exported PNG. This may be a value either between 0.0 and 1.0 (0.0 meaning full transparency, 1.0 full opacity) or between 1 and 255 (255 meaning full opacity). If not set and the -b option is not used, the page opacity set in Inkscape in the Document Options dialog will be used (stored in the inkscape:pageopacity= attribute of sodipodi:namedview). If not set but the -b option is used, the value of 255 (full opacity) will be used. 

=item B<-z>, B<--without-gui>

Do not open the GUI (on Unix, do not use X server); only process the files from console. This is assumed for -p, -e, and -l options. 
...
Ctrl, Shift, Alt modify dragging and clicking behaviour in most tools. Watch the statusbar
for helpful hints.

To rotate an object, click it to select with Selector tool, then click again to enable
rotation handles.

The Import command works with most bitmap formats (PNG, BMP, JPG, XPM, GIF etc.) and SVG
vector images. The Open command can handle only SVG files.

The Export command only exports bitmaps in PNG format. To export into PS, EPS and other
vector formats, use Save As.

A Wacom tablet will work with Inkscape if it is configured to act as a regular pointing
device. We do not support pressure/tilt sensitivity or any tablet-specific
features yet.

It works "out of the box" with Apple's X server, but with XFree86 you
will need to have the "SendCoreEvents" option turned on if you do not
already.  It does not seem to work as well under Windows.


=head1 BUGS

Many bugs are known; please refer to the website for reviewing the
reported ones and to report newly found issues.  Here are some workarounds:

When importing svg generated with ill2svg, all paths are usually grouped.
Ungroup these manually.

If you use gradients and stretch the icon of the saved file in Nautilus,
you'll notice that the gradients don't render right.  This is a
well-known problem in Nautilus/RSVG and can be worked around by
specifying the gradient as 'userspaceinuse' in the gradient editor.

See also the Known Issues section in the Release Notes for your version (file `NEWS').
...
B<Copyright (C)> 1999-2004 by Authors.  
