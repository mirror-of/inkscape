...
shapes, bezier paths, freehand drawing, multi-line text, 
...
Exported area in SVG pixels (anonymous length units normally used in Inkscape SVG).
The default is to export the entire document page.
...
The default is 72, which corresponds to 1 SVG point (pt) exporting to 1 bitmap pixel.
...
Use bitmap (XPM) icons in the GUI instead of SVG ones.
By default, Inkscape raterizes icons on-the-fly from share/inkscape/icons/icons.svg.
Use this option to turn this off if there is any problem with SVG icons. 

=item B<-y> I<VALUE>, B<--export-background-opacity>=I<VALUE>

Opacity of the background of exported PNG.
This may be a value either between 0.0 and 1.0 (0.0 meaning full transparency, 1.0 full opacity) or between 1 and 255 (255 meaning full opacity).
...
This is assumed for -p, -e, and -l options. 
