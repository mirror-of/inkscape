...
In PNG export, set the exported area in SVG pixels (anonymous length units normally used
in Inkscape SVG).  The default is to export the entire document canvas.  The point (0,0)
is the lower-left corner.

=item B<-D>, B<--export-area-drawing>

In PNG export, exported area is the entire drawing (not canvas). With this option, the
exported image will display just the visible objects of the document without margins or
cropping. Can be used in combination wtih --export-use-hints.
...
Remove all unused items from the <lt>defs<gt> section of the SVG file.  This
option has no effect on its own, but is very useful when used in conjunction
with the -l option.
